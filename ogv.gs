function dataUpdating_OGV() {
  var sheetInterface = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    `${interfaceSheetName}`
  ); // write sheet name
  var sheetOGV = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    `${ogvSheetName}`
  ); // write sheet name
  ...
  try {
    ...
    var ids = sheetOGV.getRange(1, 1, sheetOGV.getLastRow(), 1).getValues();
    var ids = ids.flat(1);
    var newRows = [];
    for (let data of allData) {
      for (let i = 0; i < data.length; i++) {
        ...
        if (index < 0) {
          ...
          sheetOGV.getRange(index + 1, 1, 1, row[0].length).setValues(row);
        }
      }
    }
    if (newRows.length > 0) {
    ...
    sheetInterface.getRange(7, 3).setValue("Succeeded");
    sheetInterface.getRange(7, 4).setValue(new Date());
  } catch (e) {
    console.log(e.toString());
    sheetInterface.getRange(7, 3).setValue("Failed");
    sheetInterface.getRange(7, 4).setValue(new Date());
  }
}
